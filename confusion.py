...
models = ['dtree_depth5', 'dtree_depth10', 'random_forest50_depth10', 'lin_logloss_sgd_stop10', 'lin_logloss_sgd_stop15', 'lin_logloss_sgd_nostop', 'gaussianNB']
...
def testModel(numHosts:int, runId:int, model_descriptor:str, file):
    ...
    for i in range(len(y)):
        if y[i] == 0 and pred[i] == 0: tn += 1
        if y[i] == 0 and pred[i] == 1: fp += 1
        if y[i] == 1 and pred[i] == 0: fn += 1
        if y[i] == 1 and pred[i] == 1: tp += 1
    
    file.write('tp,tn,fp,fn\n')
    file.write(str(tp)+','+str(tn)+','+str(fp)+','+str(fn)+'\n')

for m in models:
    avgFile_path = './test-data/' + m + '/' + str(numHosts) + '/confusion_avg' + '.csv'
    avgFile = open(avgFile_path, 'w')
    avgFile.write('tp,tn,fp,fn\n')
    tp,tn,fp,fn = 0,0,0,0
    for runId in range(7, 11):
        file_path = './test-data/' + m + '/' + str(numHosts) + '/confusion' + str(runId) + '.csv'
        ...
        fp += int(data[2])
        fn += int(data[3])
    avgFile.write(str(tp)+','+str(tn)+','+str(fp)+','+str(fn)+'\n')
    avgFile.close()